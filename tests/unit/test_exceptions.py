...
def test_catalog_error():
    ...
    render_error = CatalogRenderError(
        "OPM render failed", catalog="redhat-operator-index", version="v4.18"
    )
    ...
    parse_error = CatalogParseError(
        "Invalid JSON in catalog", catalog="community-operator-index"
    )
...
def test_operator_error():
    ...
    not_found = OperatorNotFoundError(
        "Operator not found in catalog", operator="nonexistent-operator"
    )
    ...
    invalid_channel = InvalidChannelError(
        "Channel does not exist", operator="3scale-operator", channel="nonexistent"
    )
...
def test_version_error():
    ...
    # Test VersionError
    error = VersionError(
        "Version out of range", version="4.20", min_version="4.16", max_version="4.18"
    )
    ...
    invalid = InvalidVersionError(
        "Malformed version string", version="invalid.version.format"
    )
    ...
    comparison = VersionComparisonError(
        "Cannot compare versions", version="4.18", min_version="4.x"
    )
...
def test_configuration_error():
    ...
    error = ConfigurationError(
        "Invalid configuration value", config_key="timeout", config_value=-1
    )
...
def test_file_operation_error():
    ...
    error = FileOperationError(
        "Cannot write to file", file_path="/data/operators.json", operation="write"
    )
...
def test_network_error():
    ...
    error = NetworkError(
        "Connection timeout", url="https://registry.redhat.io/v2/", status_code=504
    )
...
def test_generation_error():
    ...
    error = GenerationError("ImageSet generation failed", stage="validation")
