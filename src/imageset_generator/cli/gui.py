...
class ImageSetGeneratorGUI:
    ...
    def setup_basic_tab(self, parent):
        ...
        ocp_section = ttk.LabelFrame(
            scrollable_frame, text="OpenShift Platform Versions", padding=10
        )
        ocp_section.pack(fill="x", padx=10, pady=5)

        ttk.Label(ocp_section, text="OCP Versions (comma-separated):").pack(anchor="w")
        ttk.Entry(ocp_section, textvariable=self.ocp_versions_var, width=60).pack(
            fill="x", pady=2
        )
        ttk.Label(
            ocp_section, text="Example: 4.14.1,4.14.2,4.14.3", font=("Arial", 8)
        ).pack(anchor="w")
        ...
        channel_combo = ttk.Combobox(
            channel_frame, textvariable=self.ocp_channel_var, values=channels, width=30
        )
        ...
        ttk.Label(ops_section, text="Operators (comma-separated suggestions):").pack(
            anchor="w"
        )
        ttk.Entry(ops_section, textvariable=self.operators_var, width=60).pack(
            fill="x", pady=2
        )
        ttk.Label(
            ops_section,
            text="Example: logging,monitoring,pipelines,service-mesh",
            font=("Arial", 8),
        ).pack(anchor="w")

        # Common operators checkboxes
        ttk.Label(ops_section, text="Common Operators (click to add):").pack(
            anchor="w", pady=(10, 0)
        )
        ...
        ttk.Entry(ops_section, textvariable=self.operator_catalog_var, width=60).pack(
            fill="x", pady=2
        )

        # Output Section
        output_section = ttk.LabelFrame(
            scrollable_frame, text="Output Configuration", padding=10
        )
    ...
    def setup_advanced_tab(self, parent):
        ...
        ttk.Label(
            images_section, text="Additional Container Images (comma-separated):"
        ).pack(anchor="w")
        ttk.Entry(
            images_section, textvariable=self.additional_images_var, width=60
        ).pack(fill="x", pady=2)
        ttk.Label(
            images_section,
            text="Example: registry.redhat.io/ubi8/ubi:latest,quay.io/my-org/app:v1.0",
            font=("Arial", 8),
        ).pack(anchor="w")
        ...
        ttk.Button(
            helm_controls, text="Add Helm Chart", command=self.add_helm_chart
        ).pack(side="left")
        ttk.Button(
            helm_controls, text="Remove Selected", command=self.remove_helm_chart
        ).pack(side="left", padx=5)

        # Helm charts list
        self.helm_tree = ttk.Treeview(
            helm_section,
            columns=("name", "repository", "version"),
            show="headings",
            height=8,
        )
        ...
        helm_scroll = ttk.Scrollbar(
            helm_section, orient="vertical", command=self.helm_tree.yview
        )
    ...
    def setup_preview_tab(self, parent):
        ...
        ttk.Button(
            controls_frame, text="Generate Preview", command=self.generate_preview
        ).pack(side="left")
        ttk.Button(
            controls_frame, text="Generate & Save", command=self.generate_and_save
        ).pack(side="left", padx=5)
        ttk.Button(
            controls_frame, text="Clear Preview", command=self.clear_preview
        ).pack(side="left", padx=5)
        ...
        self.preview_text = scrolledtext.ScrolledText(
            preview_frame, height=25, width=80
        )
    ...
    def refresh_helm_tree(self):
        ...
        for chart in self.helm_charts:
            self.helm_tree.insert(
                "",
                "end",
                values=(chart["name"], chart["repository"], chart.get("version", "")),
            )
    ...
    def generate_and_save(self):
        ...
        try:
            ...
            messagebox.showinfo(
                "Success", f"ImageSetConfiguration saved to {output_file}"
            )
    ...
    def create_generator(self):
        ...
        if additional_images:
            images = [
                img.strip() for img in additional_images.split(",") if img.strip()
            ]
...
class HelmChartDialog:
    ...
    def setup_dialog(self):
        ...
        # Chart name
        ttk.Label(main_frame, text="Chart Name:").grid(
        ...
        # Repository
        ttk.Label(main_frame, text="Repository:").grid(
            row=1, column=0, sticky="w", pady=2
        )
        self.repo_var = tk.StringVar()
        ttk.Entry(main_frame, textvariable=self.repo_var, width=40).grid(
            row=1, column=1, columnspan=2, sticky="ew", pady=2
        )

        # Version
        ttk.Label(main_frame, text="Version (optional):").grid(
            row=2, column=0, sticky="w", pady=2
        )
        self.version_var = tk.StringVar()
        ttk.Entry(main_frame, textvariable=self.version_var, width=40).grid(
        ...
        ttk.Button(button_frame, text="Add", command=self.add_chart).pack(
            side="left", padx=5
        )
